...
def year_calendar_view(request, slug):
    print("Entrou na view year_calendar_view")  # Para fins de depuração

    calendar = get_object_or_404(Calendar, slug=slug)
    
    # Lista de anos disponíveis (aqui listando de 2000 a 2030 por exemplo)
    available_years = list(range(2000, 2031))
    
    # Obter o ano selecionado da query string, ou usar o ano atual como padrão
    selected_year = request.GET.get('year', timezone.now().year)
    selected_year = int(selected_year)  # Converter para inteiro

    # Ajustar o período de acordo com o ano selecionado
    start_date = datetime.datetime(selected_year, 1, 1)
    end_date = datetime.datetime(selected_year, 12, 31)
    ...
    # Depuração: Imprimir variáveis para verificar
    ...
    return render(request, 'schedule/calendar_year.html', context)
...
class EditEventView(EventEditMixin, UpdateView):
    ...
    def form_valid(self, form):
        ...
        dts = datetime.datetime.timedelta(
            minutes=int((event.start - old_event.start).total_seconds() / 60)
        )
        dte = datetime.datetime.timedelta(
    ...
    def get_initial(self):
        ...
        if date:
            try:
                ...
                initial_data = {
                    "start": start,
                    "end": start + datetime.datetime.timedelta(minutes=30),
                }
            except TypeError:
                raise Http404
            except ValueError:
...
def api_move_or_resize_by_code(request):
    ...
    delta = datetime.datetime.timedelta(minutes=int(request.POST.get("delta")))
