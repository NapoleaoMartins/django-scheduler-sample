...
urlpatterns = [
     path('year_calendar/<slug:slug>/', views.year_calendar_view, name='year_calendar_view'),
 ...
 path('', ListView.as_view(model=Calendar), name='calendar_list'),  # Lista de calendários
    path('calendar_year/<slug:slug>/', views.year_calendar_view, name='year_calendar_view'),  # Visualização do calendário por ano
    re_path(
        r'^calendar/year/(?P<calendar_slug>[-\w]+)/$',
        CalendarByPeriodsView.as_view(template_name='schedule/calendar_year.html'),
        name='year_calendar',
